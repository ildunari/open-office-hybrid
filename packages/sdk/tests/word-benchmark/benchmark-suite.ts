...
export interface WordBenchmarkRunSummary {
  ...
  failureCounts: Record<WordBenchmarkFailureKind, number>;
  failedTaskGroups: Record<WordBenchmarkFailureKind, string[]>;
...
export type WordBenchmarkFailureKind =
  | "prompt_timeout"
  | "bridge_refresh_timeout"
  | "session_collision"
  | "other";

export interface WordBenchmarkFailureSummary {
  kind: WordBenchmarkFailureKind;
  message: string;
}

export interface WordBenchmarkTaskRunPolicy {
...
export function classifyWordBenchmarkFailure(
  message: string | null | undefined,
): WordBenchmarkFailureSummary {
...
export function buildWordBenchmarkTaskRunPolicy(
  task: WordBenchmarkTask | WordBenchmarkMultistepSession,
): WordBenchmarkTaskRunPolicy {
  const taskId = "task_id" in task ? task.task_id : task.session_id;
  const isRecoveryOrScopeSensitive =
    taskId.startsWith("M") ||
    taskId.startsWith("S") ||
    ("mutation_ids" in task && (task.mutation_ids?.length ?? 0) > 0);

  return {
    promptTimeoutMs: isRecoveryOrScopeSensitive ? 600_000 : 300_000,
...
export function summarizeWordBenchmarkRun(
  ...
  const failureCounts: Record<WordBenchmarkFailureKind, number> = {
    bridge_refresh_timeout: 0,
    prompt_timeout: 0,
    session_collision: 0,
    other: 0,
  };
  const failedTaskGroups: Record<WordBenchmarkFailureKind, string[]> = {
    bridge_refresh_timeout: [],
    prompt_timeout: [],
    session_collision: [],
    other: [],
